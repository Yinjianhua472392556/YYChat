...
@interface YYMessageCell()<IModelChatCell> {

    YYMessageBodyType _messageType;
}

@property (nonatomic) NSLayoutConstraint *statusWidthConstraint;
@property (nonatomic) NSLayoutConstraint *activityWidthConstraint;
...
+ (void)initialize {
    ...
    YYMessageCell *cell = [self appearance];
    cell.statusSize = 20;
...
//    cell.recvBubbleBackgroundImage = [UIImage imageNamed:@"sendBackgroundImage"];
//    cell.sendBubbleBackgroundImage = [UIImage imageNamed:@"receiveBackgroundImage"];
}
...
- (void)_setupSubviewsWithType:(YYMessageBodyType)messageType isSender:(BOOL)isSender model:(id<IMessageModel>)model {
    ...
    _bubbleView = [[YYBubbleView alloc] initWithMargin:isSender?_rightBubbleMargin:_leftBubbleMargin isSender:isSender];
...
- (void)_setupConstraints {
    ...
    //activtiy
    self.activityWidthConstraint = [NSLayoutConstraint constraintWithItem:self.activity attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:self.activitySize];
    [self addConstraint:self.activityWidthConstraint];
...
- (void)_updateActivityWidthConstraint {
    ...
    if (_activity) {
        [self removeConstraint:self.activityWidthConstraint];
...
- (void)setModel:(id<IMessageModel>)model {
    ...
    if ([self respondsToSelector:@selector(isCustomBubbleView:)] && [self isCustomBubbleView:model]) {
        ...
        switch (model.typefile) {
            case YYMessageBodyTypeText: {
                _bubbleView.textLabel.attributedText = [[EaseEmotionEscape sharedInstance] attStringFromTextForChatting:model.content textFont:self.messageTextFont];
                break;
            }
            case YYMessageBodyTypeImage: {
...
//                    _bubbleView.imageView.image = image;
//                }
                [_bubbleView.imageView sd_setImageWithURL:[NSURL URLWithString:_model.image.urlsmall] placeholderImage:[UIImage imageNamed:@"placeHoldHeader"]];
                break;
            }
            case YYMessageBodyTypeVideo: {
...
//                }
            ...
            case YYMessageBodyTypeLocation: {
                _bubbleView.locationLabel.text = _model.location.address;
                break;
            }
            case YYMessageBodyTypeVoice: {
                if (_bubbleView.voiceImageView) {
                    if ([self.sendMessageVoiceAnimationImages count] > 0 && [self.recvMessageVoiceAnimationImages count] > 0) {
                        ...
                        self.bubbleView.voiceImageView.animationImages = self.model.isSender? self.sendMessageVoiceAnimationImages : self.recvMessageVoiceAnimationImages;
                    } else {
...
//                if (!self.model.isSender) {
                    _bubbleView.isReadView.hidden = YES; //先隐藏
...
//                }
                ...
                _bubbleView.voiceDurationLabel.text = [NSString stringWithFormat:@"%@''",_model.voice.time];
...
- (void)statusAction {
    ...
    if ([_delegate respondsToSelector:@selector(statusButtonSelected:withMessageCell:)]) {
...
- (void)avatarViewTapAction:(UITapGestureRecognizer *)tapRecognizer {
    ...
    if ([_delegate respondsToSelector:@selector(avatarViewSelected:)]) {
        [_delegate avatarViewSelected:_model];
    }
}
...
+ (CGFloat)cellHeightWithModel:(id<IMessageModel>)model {
    ...
    switch (model.typefile) {
        case YYMessageBodyTypeText: {
            NSAttributedString *text = [[EaseEmotionEscape sharedInstance] attStringFromTextForChatting:model.content textFont:cell.messageTextFont];
...
//            CGFloat height = [model.image.height floatValue];
            CGSize retSize = CGSizeMake(kEMMessageImageSizeWidth, kEMMessageImageSizeHeight);
