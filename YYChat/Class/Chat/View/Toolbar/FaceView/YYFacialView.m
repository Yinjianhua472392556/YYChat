...
@protocol EaseCollectionViewCellDelegate <NSObject>
...
- (void)sendEmotion:(id)sender {
    if ([_delegate respondsToSelector:@selector(didSendEmotion:)]) {
...
@interface YYFacialView()<UIScrollViewDelegate,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,EaseCollectionViewCellDelegate> {
    CGFloat _itemWidth;
    CGFloat _itemHeight;
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        [_collectionView registerClass:[EaseCollectionViewCell class] forCellWithReuseIdentifier:@"collectionCell"];
...
- (void)didSendEmotion:(EaseEmotion *)emotion {
    if (emotion) {
        if (emotion.emotionType == EMEmotionDefault) {
            if ([_delegate respondsToSelector:@selector(selectedFacialView:)]) {
...
- (void)loadFacialViewWithPage:(NSInteger)page {
    [_collectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:0 inSection:page] atScrollPosition:UICollectionViewScrollPositionCenteredHorizontally animated:NO];
