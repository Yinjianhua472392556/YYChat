...
@interface YYChatBarMoreView()<UIScrollViewDelegate> {

    YYChatToolbarType _type;
    NSInteger _maxIndex;
}
...
+ (void)initialize {
    ...
    YYChatBarMoreView *moreView = [self appearance];
...
- (void)setupSubviewsForType:(YYChatToolbarType)type {
    ...
    else if (type == YYChatToolbarTypeGroup) {
...
- (void)moreAction:(id)sender
{
    ...
    if (button && _delegate && [_delegate respondsToSelector:@selector(moreView:didSelectItemInMoreViewAtIndex:)]) {
        [_delegate moreView:self didSelectItemInMoreViewAtIndex:button.tag-MOREVIEW_BUTTON_TAG];
    }
}
