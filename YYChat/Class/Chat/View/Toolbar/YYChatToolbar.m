...
@interface YYChatToolbar()<UITextViewDelegate, YYFaceViewDelegate>
 ...
 *  底部扩展页面
 */
@property (nonatomic) BOOL isShowBottomView;
@property (nonatomic, strong) UIView *activityBottomView; //当前活跃的底部扩展页面
...
@property (nonatomic, weak) NSLayoutConstraint *inputViewWidthItemLeftConstraint;
@property (nonatomic, weak) NSLayoutConstraint *inputViewWidthoutItemsLeftConstraint;



@end

@implementation YYChatToolbar
...
- (instancetype)initWithFrame:(CGRect)frame {
    self = [self initWithFrame:frame horizontalPadding:8 verticalPadding:5 inputViewMinHeight:36 inputViewMaxHeight:150 type:YYChatToolbarTypeGroup];
    if (self) {
...
- (instancetype)initWithFrame:(CGRect)frame horizontalPadding:(CGFloat)horizontalPadding verticalPadding:(CGFloat)verticalPadding inputViewMinHeight:(CGFloat)inputViewMinHeight inputViewMaxHeight:(CGFloat)inputViewMaxHeight type:(YYChatToolbarType)type {
...
- (void)_setupSubviews {
    ...
    //输入框
    _inputTextView = [[YYTextView alloc] initWithFrame:CGRectMake(self.horizontalPadding, self.verticalPadding, self.frame.size.width - self.verticalPadding * 2, self.frame.size.height - self.verticalPadding * 2)];
    _inputTextView.autoresizingMask = UIViewAutoresizingFlexibleHeight;
    _inputTextView.scrollEnabled = YES;
    _inputTextView.returnKeyType = UIReturnKeySend;
    _inputTextView.enablesReturnKeyAutomatically = YES; // UITextView内部判断send按钮是否可以用
    _inputTextView.placeHolder = @"输入消息...";
    _inputTextView.delegate = self;
    _inputTextView.backgroundColor = [UIColor clearColor];
    _inputTextView.layer.borderColor = [UIColor colorWithWhite:0.8f alpha:1.0f].CGColor;
    _inputTextView.layer.borderWidth = 0.65f;
    _inputTextView.layer.cornerRadius = 6.f;
    ...
    YYChatToolbarItem *styleItem = [[YYChatToolbarItem alloc] initWithButton:styleChangeButton withView:nil];
    ...
    YYChatToolbarItem *faceItem = [[YYChatToolbarItem alloc] initWithButton:self.faceButton withView:self.faceView];
    ...
    YYChatToolbarItem *moreItem = [[YYChatToolbarItem alloc] initWithButton:self.moreButton withView:self.moreView];
...
- (void)setDelegate:(id)delegate {
    ...
    _delegate = delegate;
    if ([_moreView isKindOfClass:[YYChatBarMoreView class]]) {
        [(YYChatBarMoreView *)_moreView setDelegate:delegate];
    }
}
...
- (void)setMoreView:(UIView *)moreView {
    ...
    if (_moreView != moreView) {
        ...
        for (YYChatToolbarItem *item in self.rightItems) {
...
- (void)setInputViewLeftItems:(NSArray *)inputViewLeftItems {
    ...
    for (YYChatToolbarItem *item in self.leftItems) {
    ...
    CGFloat originalX = self.horizontalPadding;
    CGFloat itemHeight = self.toolbarView.frame.size.height - self.verticalPadding * 2;
    
    for (id item in inputViewLeftItems) {
        if ([item isKindOfClass:[YYChatToolbarItem class]]) {
            YYChatToolbarItem *chatItem = (YYChatToolbarItem *)item;
            if (chatItem.button) {
    ...
    CGFloat value = inputFrame.origin.x - originalX;
    inputFrame.origin.x = originalX;
...
- (UIView *)recordView {
    ...
    if (_recordView == nil) {
        _recordView = [[YYRecordView alloc] initWithFrame:CGRectMake(90, 130, 140, 140)];
    }
    return _recordView;
}
...
- (UIView *)faceView {
    ...
    if (_faceView == nil) {
        _faceView = [[YYFaceView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_toolbarView.frame), self.frame.size.width, 180)];
        [(YYFaceView *)_faceView setDelegate:self];
...
- (UIView *)moreView {
    if (_moreView == nil) {
        _moreView = [[YYChatBarMoreView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_toolbarView.frame), self.frame.size.width, 80) type:self.chatBarType];
...
- (void)faceButtonAction:(id)sender {
    ...
    YYChatToolbarItem *faceItem = nil;
    for (YYChatToolbarItem *item in self.rightItems) {
        if (item.button == button) {
            faceItem = item;
            continue;
...
- (void)moreButtonAction:(id)sender {
    ...
    YYChatToolbarItem *moreItem = nil;
    for (YYChatToolbarItem *item in self.rightItems) {
        if (item.button == button){
            moreItem = item;
            continue;
...
- (void)recordButtonTouchUpInside
{
    ...
    if ([self.delegate respondsToSelector:@selector(didFinishRecodingVoiceAction:)])
    {
        [self.delegate didFinishRecodingVoiceAction:self.recordView];
    }
...
#pragma mark - YYFaceViewDelegate
    ...
    if (chatText.length > 0) {
        if ([self.delegate respondsToSelector:@selector(didSendText:)]) {
            if (![_inputTextView.text isEqualToString:@""]) {
                ...
                [_inputTextView.attributedText enumerateAttribute:NSAttachmentAttributeName inRange:NSMakeRange(0, self.inputTextView.attributedText.length) options:NSAttributedStringEnumerationReverse usingBlock:^(id  _Nullable value, NSRange range, BOOL * _Nonnull stop) {
                    if (value) {
